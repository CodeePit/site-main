...
const FormLogin = () => {
  ...
  return (
    <form
      onSubmit={onSubmit}
      className="w-full h-full flex justify-center items-center text-left ml-10 max-md:ml-0"
    >
      <div className="w-[70%] max-2xl:w-[90%] flex flex-col h-full justify-center relative max-md:w-full">
        ...
        <div className="w-full flex flex-col gap-3 max-md:mt-10">
          ...
          <span className="w-[80%] max-xl:w-full  text-[#141624]/60 text-base max-lg:text-sm font-medium">
            Hoje é um novo dia. É o teu dia. Você molda isso. Faça login para começar a
            gerenciar seus projetos.
          </span>
          <div className="w-[80%] max-xl:w-full mt-6 flex flex-col gap-3">
            <div className="flex flex-col w-full gap-2">
              <Label htmlFor="email" className="font-bold text-base max-sm:text-sm">
                Email
              </Label>
              <input
                type="email"
                ...
                id="email"
                placeholder="Email"
                className="w-full rounded-lg  py-3 px-3 border border-input bg-background"
              />
            </div>

            <div className="flex flex-col w-full gap-2">
              <Label htmlFor="senha" className="font-bold text-base max-sm:text-sm">
                Senha
              </Label>
              <div className="w-full rounded-lg border flex items-center border-input bg-background relative">
                <input
                  ...
                  id="senha"
                  placeholder="Senha"
                  className="w-full px-3 pr-12 rounded-lg h-full py-3"
                />
                {visiblePass ? (
                  <EyeIcon
                    className="cursor-pointer absolute right-4 color_svg"
            ...
            <button
              disabled={loading || isLoading}
              className={clsx(
                'w-full bg-yellow-400 py-3 mt-5 text-[#141624]/80 text-lg max-sm:text-base font-bold rounded-lg hover:bg-yellow-400/70',
                { 'bg-yellow-400/70 animate-pulse': isLoading || loading },
              )}
            >
              Entrar
            </button>
