    ...
    <HeroText.Root className="header_process">
      <HeroText.Img className="w-[53%] h-full top-[-53%] max-xl:top-[-50%] max-sm:top-[-45%] !shadow-none max-lg:opacity-[20%]  !rounded-none right-[-50%]  translate-x-[-85%] translate-y-[40%] max-[1700px]:w-[50%] max-xl:w-[80%] max-xl:translate-x-[-30%]">
        <div className="w-full h-full rounded-full relative z-50">
          ...
          >
            ...
            >
              <Image src={Process} alt="PC" width={2525} height={2278} />
            </motion.div>
          </motion.div>
        </div>
        <div className="w-[20rem] h-[20rem] rounded-full sombra-6 absolute z-0 bottom-[1rem] left-[0] max-md:top-[10rem] max-md:right-0 max-sm:hidden" />
        <div className="w-[20rem] h-[20rem] rounded-full sombra-7 absolute z-0 bottom-[1rem] right-[30%] max-md:bottom-[0rem] max-md:left-0 max-sm:hidden" />
      </HeroText.Img>
      <motion.div
      ...
      >
        <HeroText.Title className="max-2xl:max-w-[40rem] max-2xl:text-[2.5rem] max-xl:text-[2rem] max-lg:text-[3rem] max-sm:text-[1.5rem]">
          Codepit Web <br /> & Mobile
        </HeroText.Title>
        <HeroText.SubTitle>
          Seja desenvolvimento web ou mobile nós encontramos e desenvolvemos a solução
          para o seu problema! De maneira performatica atualizada e muito elegante você
          tera suas soluções unicas e costumizadas.
        </HeroText.SubTitle>
        <HeroText.Buttons>
          <Button
            fill
          ...
          <Button href="https://wa.me/5515988045279?text=Ol%C3%A1%2C+vim+do+site+Codepit.">
