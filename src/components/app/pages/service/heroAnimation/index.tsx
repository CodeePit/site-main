    ...
    >
      <div className="flex flex-col max-md:justify-start max-md:items-center gap-[2rem]">
        <h1 className="max-w-[33rem] max-md:text-center max-md:max-w-[95%] max-sm:max-w-[100%] font-bold text-[2rem] leading-none max-sm:leading-tight max-[1600px]:max-w-[30rem] max-2xl:text-[2.5rem] max-xl:text-[2rem] max-xl:max-w-[40%] max-sm:text-[1.5rem]">
          Entre em contato e <span className="text-[#5956E8]">comece</span>{' '}
          <span className="text-[#5956E8]">crescendo </span>sua startup hoje mesmo!
        </h1>
        <div className="flex items-center gap-[1rem]">
          <Button
            href="https://api.whatsapp.com/send/?phone=5515988045279&text=Ol%C3%A1%2C+vim+do+site+Codepit.&type=phone_number&app_absent=0"
            fill
          >
            Saber mais
          </Button>
          <Button href="https://api.whatsapp.com/send/?phone=5515988045279&text=Ol%C3%A1%2C+vim+do+site+Codepit.&type=phone_number&app_absent=0">
            Começar agora
          </Button>
        </div>
      </div>
      <motion.div className="w-full max-md:h-[20rem]">
        {newVariant && newVariantFoguete && (
          <motion.div
            variants={newVariant}
            className="absolute -right-40 -top-5 w-[35rem] h-[30rem] max-md:w-[30rem] max-md:h-[30rem] rounded-full bg-white max-md:right-[50%] max-md:!translate-x-[50%] max-md:top-[initial] max-md:!bottom-[-150px]  "
          >
            <motion.div variants={newVariant} className="w-full h-full relative">
              <div className="w-full h-full sombra-hero absolute" />
              <Image
                src={Fundo}
