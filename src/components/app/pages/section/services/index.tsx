...
const ServicesSection = ({ isPage }: { isPage?: boolean }) => {
  return (
    ...
    >
      <div
        className={clsx('w-[80%] h-full flex flex-col gap-[4rem] max-2xl:w-full', {
          'items-center': isPage,
        })}
      >
        <HeaderSection.Root>
          <div
            className={clsx('flex items-center gap-[2rem] max-md:gap-[1rem]', {
              'flex-col text-center': isPage,
            })}
          >
            ...
            <div
              className={clsx('flex flex-col gap-[.5rem]', { 'items-center': isPage })}
            >
              <HeaderSection.Path>
              ...
              <HeaderSection.Title asChild>
                <h1 className="text-[1.4rem] text-white/90 font-medium leading-tight max-lg:max-w-[80%] max-lg:text-[1rem] max-md:w-full max-sm:text-[.9rem]">
                  Serviços de alto impacto para levar sua empresa ao próximo nível
                </h1>
              </HeaderSection.Title>
            </div>
          </div>
