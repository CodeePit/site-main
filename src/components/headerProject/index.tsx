...
const HeaderProject = ({ projeto }: { projeto: iProjects }) => {
  return (
    <motion.section
      className={
        'w-full h-fit flex py-[120px] pb-[60px] justify-center min-h-[25vh]  relative'
      }
      style={{ background: projeto.bgColor }}
    >
        ...
        >
          <div className="w-full flex flex-col items-center">
            <Image
              src={projeto.img}
              alt={projeto.title}
              className="w-full mt-[.5rem] h-[30rem] shadow-lg rounded-2xl object-cover max-[1100px]:h-[30rem]"
            />
          </div>
          <div className="w-full justify-between flex items-center max-sm:grid-cols-2 max-sm:!grid max-sm:grid-rows-auto justify-items-center max-sm:gap-[1rem] ">
            <div className="flex flex-col gap-[.2rem] max-sm:items-center">
              ...
              <span className="text-secondary_black-950/70 max-xl:text-[.8rem]">
                {projeto.client}
              </span>
            </div>
            <div className="flex flex-col gap-[.2rem] max-sm:items-center">
        ...
        >
          ...
          <div className="flex gap-[2rem] max-sm:justify-center flex-wrap">
            {projeto.stacks.map((stacks) => (
              <div
                key={stacks.id}
                className="flex flex-col items-center gap-[.7rem] transition-all hover:scale-[1.1]"
              >
                <Image
                  src={stacks.icon}
                  alt={stacks.name}
                ...
                />
                <span className="text-secondary_black-950/70 text-[.7rem]">
                  {stacks.name}
                </span>
              </div>
            ))}
          ...
          {projeto.link && (
            <Button href={projeto.link} className="w-fit text-[.8rem]" fill>
              Acessar página <IconProx white />
            </Button>
          )}
        </motion.div>
      </motion.div>
